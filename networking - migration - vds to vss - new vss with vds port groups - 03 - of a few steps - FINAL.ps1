        ...
        foreach($vm in $vmlist)
            ...
            foreach($adapter in $AdapterList)
                ...
                foreach($row in $report)
                    ...
                    If ($cluster -eq $row.cluster -and $vmhost -eq $row.movinghost -and $adapter.NetworkName -match $row.olddvPG)
                        {
                            Write-Host "`tAdapter Name:" $adapter.Name
                            Write-Host "`t`tCurrently" $adapter.Name "has portgroup" $adapter.NetworkName "and that matches the Excel Row of olddvPG to" $row.olddvPG
                            Write-Host "`t`tSo," $vm $adapter.Name "will be set to the new vSwitch0 vm portgroup of" $row.tmpvPG
                            Write-Host "`t`tThus, I'll do my set here."
                            Write-Host "`t`tSetting VM" $vm $adapter.Name "to the new vSwitch0 port of" $row.tmpvPG
